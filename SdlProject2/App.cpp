...
void App::GameScene()
{
    ...
    while (!gameEnded)
    {
        ...
        while (SDL_PollEvent(&Engine::e) != 0) 
        {
            ...
            if (Engine::e.type == SDL_KEYDOWN)
            {
                if (Engine::e.key.keysym.sym == SDLK_l)
                {

                    Engine::largerKeyboard = true;
                }
                if (Engine::e.key.keysym.sym == SDLK_s)
                {
                    Engine::smallerKeyboard = true;
                }
            }
            if (Engine::e.type == SDL_KEYUP)
            {
                if (Engine::e.key.keysym.sym == SDLK_l)
                {
                    Engine::largerKeyboard = false;
                }
                if (Engine::e.key.keysym.sym == SDLK_s)
                {
                    Engine::smallerKeyboard = false;
                }
            }
            if (Engine::e.type == SDL_MOUSEBUTTONDOWN)
            {
